...
extern "x86-interrupt" fn segment_not_present(stack: InterruptStackFrame, err_code: u64) {
    log::debug!("{:#x?}", stack);
    panic!(
        "Attempted to load not present segment with index {:x} (#NP)",
        err_code
    );
}
...
extern "x86-interrupt" fn gpf(stack: InterruptStackFrame, err_code: u64) {
    log::debug!("{:#x?}", stack);
    panic!(
        "General Protection Fault (#GP) with error code {:x}",
        err_code
    );
}
...
extern "x86-interrupt" fn control_protection(stack: InterruptStackFrame, err_code: u64) {
    log::debug!("{:#x?}", stack);
    panic!(
        "Control Protection Exception (#CP) with error code {:x}",
        err_code
    );
}
...
extern "x86-interrupt" fn vmm_comm(stack: InterruptStackFrame, err_code: u64) {
    log::debug!("{:#x?}", stack);
    panic!(
        "VMM Communication Exception (#VC) with error code {:x}",
        err_code
    );
}
